...
`;
  ...
  /**
   * Create Jest configuration file (jest.config.cjs)
   * Uses .cjs extension because package.json has "type": "module" (ESM)
   * This is separate from package.json to avoid conflicts
   */
  async createJestConfig(projectBasePath: string, techStack?: string): Promise<void> {
    const stackType = this.detectStackType(techStack);

    // Only create jest.config.cjs for Node.js/TypeScript projects
    if (stackType !== 'nodejs') {
      return;
    }

    // Use .cjs extension because package.json uses "type": "module"
    const jestConfigPath = path.join(projectBasePath, 'jest.config.cjs');

    // Check if jest.config.cjs already exists
    try {
      await fs.access(jestConfigPath);
      console.log('[ProjectStructure] jest.config.cjs already exists, skipping creation');
      return;
    } catch {
      // jest.config.cjs doesn't exist, create it
    }
...
`;
    ...
    try {
      ...
      console.log('[ProjectStructure] ✅ Created jest.config.cjs');
    } catch (error: any) {
      console.error('[ProjectStructure] Error creating jest.config.cjs:', error.message);
      throw error;
    }
  }
...
`;
   ...
   * Removes jest config from package.json if jest.config.js or jest.config.cjs exists
   */
  async cleanupJestConfigConflict(projectBasePath: string): Promise<void> {
    const packageJsonPath = path.join(projectBasePath, 'package.json');
    const jestConfigJsPath = path.join(projectBasePath, 'jest.config.js');
    const jestConfigCjsPath = path.join(projectBasePath, 'jest.config.cjs');
    ...
    try {
      ...
      // Check if either jest config file exists
      const jestConfigJsExists = await fs.access(jestConfigJsPath).then(() => true).catch(() => false);
      const jestConfigCjsExists = await fs.access(jestConfigCjsPath).then(() => true).catch(() => false);

      if (!jestConfigJsExists && !jestConfigCjsExists) {
        return; // No jest config file
      }
      ...
      if (packageJson.jest) {
        const configType = jestConfigCjsExists ? 'jest.config.cjs' : 'jest.config.js';
        console.log(`[ProjectStructure] ⚠️ Found jest config in package.json and ${configType} - removing from package.json`);
        delete packageJson.jest;

        // Write back without jest config
        await fs.writeFile(packageJsonPath, JSON.stringify(packageJson, null, 2), 'utf8');
        console.log(`[ProjectStructure] ✅ Removed jest config from package.json (using ${configType} instead)`);
      }
    } catch (error: any) {
