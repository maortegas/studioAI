...
import { JestConfigGenerator } from '@devflow-studio/shared';
...
`;
    ...
    try {
      ...
      const jestResult = await JestConfigGenerator.autoConfigureJest(projectBasePath);
      if (jestResult.configured) {
        console.log(`[ProjectStructure] ✅ ${jestResult.reason}`);
      } else {
        console.log(`[ProjectStructure] Jest auto-config skipped: ${jestResult.reason}`);
      }
