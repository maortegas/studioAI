                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      ...
                      <div className="flex items-center space-x-2">
                        {session.status === 'generating_tests' && (
                          <span className="flex items-center space-x-1 px-3 py-2 text-sm bg-yellow-100 text-yellow-700 rounded-lg border border-yellow-300">
                        ...
                        {session.status === 'tests_generated' && (
                          <span className="flex items-center space-x-1 px-3 py-2 text-sm bg-green-100 text-green-700 rounded-lg border border-green-300">
                            <span>✅</span>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div className="relative w-full bg-gray-200 rounded-full h-2">
                        ...
                        {((session as any).test_progress || 0) > 0 && (
                          <div
                            className="absolute left-0 top-0 h-2 rounded-full bg-yellow-500 transition-all duration-500"
                            style={{ width: `${((session as any).test_progress || 0) * 2}%` }}
                          />
                        )}
                        {/* Implementation progress (50-100%) */}
                        {((session as any).implementation_progress || 0) > 0 && (
                          <div
                            className="absolute left-0 top-0 h-2 rounded-full bg-blue-500 transition-all duration-500"
                            style={{ 
                              left: '50%',
                              width: `${((session as any).implementation_progress || 0) * 2}%` 
                            }}
                          />
                        )}
                        {/* Fallback to overall progress */}
                        {!((session as any).test_progress && (session as any).implementation_progress) && (
                          <div
